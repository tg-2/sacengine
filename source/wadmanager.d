...
class WadManager{
	...
	void indexWAD(alias filenameCallback=0,T...)(string wadPath,string dirPath,T args){
		...
		while(info.length){
			...
			auto filename=buildPath(odir,name.replace("/","_SLASH_").replace("\\","_BSLASH_").replace("`","_TICK_").replace("\0","_ZERO_").replace("^","_HAT_").replace("/","_SLASH_").replace("?","_QUESTIONMARK_"));
