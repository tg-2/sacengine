...
SuperImage loadTXTR(string filename){ // TODO: maybe integrate with dagon's TextureAsset
	enforce(filename.endsWith(".TXTR")||filename.endsWith(".ICON")||filename.endsWith(".MAPT"));
	auto base = filename[0..$-".TXTR".length];
	ubyte[] txt;
	foreach(ubyte[] chunk;chunks(File(filename,"rb"),4096)) txt~=chunk;
	uint width,height;
	string pal;
	if(filename.endsWith(".MAPT")){
		width=heigth=64;
		pal="LAND";
	}else{
		width=parseLE(txt[0..4]);
		txt=txt[4..$];
		height=parseLE(txt[0..4]);
		txt=txt[4..$];
		auto tmp=txt[0..4].dup;
		txt=txt[4..$];
		reverse(tmp);
		pal=cast(string)tmp.idup;
	}
